...
\paragraph{The or-to-if argument} A second justification for symbolising `if' as `$\eif$' is this. We know already that if \meta{A} is true and \meta{C} is false, then `if \meta{A} then \meta{C}' will be false. What should we say about the other rows of the truth table, the rows on which either \meta{A} is false, or \meta{B} is true (or both). Well, suppose that either \meta{A} is false or \meta{B} is true. If the first disjunct of this disjunction is false, then the second disjunct is true. The first disjunct (`\meta{A} is false') is false just in case \meta{A}; so it turns out that if \meta{A}, then \meta{B}. so `if \meta{A}, then \meta{B}' should hold on those other three lines, giving us the truth table we have associated with `$\eif$' as the best truth table for `if'.
