...
It is important to remember that these conventions are not modifications of our rules concerning which expressions are sentences (§\ref{s.sentencesTFL}). Rather, these conventions allow us to unambiguously reconstruct a sentence from an expression. `$A \vee B$' is not a sentence, but in the right circumstances it can be used to represent the sentence `$(A \vee B)$'. If you find the conventions confusing or unhelpful, you do not have to use them.

There are systems of logic which omit parentheses entirely, using so-called `Polish notation'. I discuss this notation briefly in Appendix \ref{app.notation}, p.~\pageref{polish}. 
