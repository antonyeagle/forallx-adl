...
Let's return to our example `Barry is taller than everyone else'. We want to start with a paraphrase: pick anyone; if they are not Barry, than Barry is taller than them. Where $b$ symbolises `Barry' and $T$ symbolises `\gap{1} is taller than \gap{2}', we want to symbolise this as: $\forall x (\enot x = b \eif Tbx)$ (on the domain of people). 
...
Now w can symbolise `there are at least $n$', and we can symbolise `there are at most $n$'. Using them together, we can symnbolise `there are exactly $n$':
\begin{earg}
