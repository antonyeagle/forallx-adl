...
Suppose we have a natural deduction proof. But what is it a proof \emph{of}? Well, the last sentence in the sequence is in some sense where we've ended up:  the result of the proof. It can be considered the conclusion of the proof. But even if there is a proof with conclusion \meta{C}, that doesn't mean that we have proved \meta{C}, let alone that we should therefore come to believe it. For we have neglected the role of assumptions. Some of the proof rules discharge previously made assumptions, but not every assumption has to be discharged in a correctly formed proof. These remaining `active' assumptions – which we term \define{undischarged} assumptions – are the suppositions on which the correctness of the proof conclusion depends. So a natural deduction proof is \emph{hypothetical}: it establishes the conclusion, assuming the truth of any hypothesised undischarged assumptions. 

This structure establishes a nice relationship between proofs and arguments. \factoidbox{A given correctly formed proof  is a \define{proof of an argument} $\meta{A}_{1} … \meta{A}_{n} \ttherefore \meta{C}$ iff \begin{enumerate}
	\item the final sentence in the proof is \meta{C}, and 
	\item each of the undischarged assumptions in the range of which \meta{C} falls is \emph{among} the premises of the argument $\meta{A}_{i}$. 
\end{enumerate}}Note that according to this definition if there is a proof of $\meta{A} \ttherefore \meta{C}$, that will also be a proof of $\meta{A},\meta{B} \ttherefore \meta{C}$ – and indeed it will be a proof of any other argument with the conclusion \meta{C} and including \meta{A} as a premise. This is related to the fact that if you have a valid argument, adding extra premises can't make the argument invalid. (Likewise, if you have a correctly constructed proof, making additional assumptions above the first line can't make it incorrect.) 
...
Whenever we make a new assumption, we underline it and introduce a new assumption line. So this is a proof corresponding to the argument $P, Q \ttherefore Q$: \begin{proof}
	\hypo{p} P
	\open
	\hypo{q} Q
\end{proof}
...
\problempart
Which of the following could, given the right rules, be turned into a proof representing the argument $$\enot(P \eand Q), P \ttherefore \enot Q?$$ (Bear in mind that many proofs can represent the same argument, and many arguments can be represented by the same proof.)
\begin{multicols}{2}\noindent
	   \begin{enumerate}
    	\item\begin{proof}
...
\begin{earg}
\item[] $\Bigl(\bigl( (A\eor B)\eif(C\eor D) \bigr) \eand \bigl( (E \eor F) \eif (G\eor H) \bigr)\biggr)$
\item[\ttherefore] $\Bigl(\bigl( (E \eor F) \eif (G\eor H) \bigr) \eand \bigl( (A\eor B)\eif(C\eor D) \bigr)\biggr)$
...
\begin{proof}
	\hypo{ab}{\Bigl(\bigl((A\eor B)\eif(C\eor D)\bigr) \eand \bigl((E \eor F) \eif (G\eor H)\bigr)\biggr)}
\end{proof}
From the premise, we can get each of its conjuncts by {\eand}E. The proof now looks like this:
\begin{proof}
	\hypo{ab}{\Bigl(\bigl((A\eor B)\eif(C\eor D)\bigr) \eand \bigl((E \eor F) \eif (G\eor H)\bigr)\biggr)}
	\have{a}{\bigl((A\eor B)\eif(C\eor D)\bigr)} \ae{ab}
	\have{b}{\bigl((E \eor F) \eif (G\eor H)\bigr)} \ae{ab}
\end{proof}
...
\begin{proof}
	...
	\have{a}{\bigl((A\eor B)\eif(C\eor D)\bigr)} \ae{ab}
	\have{b}{\bigl((E \eor F) \eif (G\eor H)\bigr)} \ae{ab}
	\have{ba}{\Bigl(\bigl((E \eor F) \eif (G\eor H)\bigr) \eand \bigl((A\eor B)\eif(C\eor D)\bigr)\biggr)} \ai{b,a}
\end{proof}
...
The correctness of our proof rule of conditional elimination is supported by the easily demonstrated validity of the corresponding entailment: \begin{itemize}
	\item  $(\meta{A}\eif\meta{B}), \meta{A} \entails \meta{B}$.
...
Note the way that our conjunction and conditional elimination rules can be used to parallel the biconditional elimination rules, substantiating our intuition that a biconditional amounts to the left-to-right and right-to-left conditionals:

\begin{minipage}{0.4\textwidth} \begin{proof}
...
\begin{proof}
	\hypo{m}{M}
	\have{mmm}{\biggl(M \eor \Bigl( \bigl( (A\eiff B) \eif (C \eand D)\bigr)  \eiff \bigl( E \eand F\bigr) \Bigr)\biggr)}\oi{m}
\end{proof}
...
Because every proof begins with an assumption, we can only obtain a proof of a theorem if we discharge that opening assumption with a rule which allows one to close a subproof: conditional or biconditional introduction, or either of the negation rules (introduction or elimination): 
  \begin{proof}
    ...
    \have{p2}{P}\ae{qp}
    \close
    \have{pq}{P \eiff (P \eand Q)}\bi{p-pq,qp-p2}
    \close
    \have{qpq}{Q \eif (P \eiff (P\eand Q))}\ci{q-pq}
  \end{proof}
There is a connection to the deduction theorem here too. Any correct proof of \meta{C} with one undischarged assumption \meta{A} will demonstrate $\meta{A}\proves\meta{C}$. The deduction theorem then assures us that $\proves \meta{A}\eif\meta{C}$. We see just this in the last line of the above proof, where a proof that $Q \proves (P \eiff (P\eand Q))$ is converted to a proof showing that $\proves Q \eif (P \eiff (P\eand Q))$.
...
The colourfully named principle of \define{explosion} is the rule that from a contradiction, anything can be derived. The name reflects the disaster that it would be if a contradiction were true. This principle is sometimes called by its medieval Latin name \idefine{ex falso quodlibet} – `from a (logical) falsehood, anything'. The rule takes the following form:
\factoidbox{
	\begin{proof}
		\have[\ ]{}{\vdots}
		\have[j]{c1}{\meta{A}}
		\have[\ ]{}{\vdots}
		\have[k]{b}{\enot\meta{A}}
		\have[\ ]{}{\vdots}
		\have[l]{c}{\meta{B}}\by{EX}{c1,b}
	\end{proof}
	}
Again, explosion can be emulated using our existing rules (note, in light of §\ref{construct}, the use of $\enot$E):
\begin{proof}
	\have{a}{\meta{A}}
	\have{b}{\enot\meta{A}}
	\open
		\hypo{c}{\enot\meta{B}}
		\have{ar}{\meta{A}}\by{R}{a}
		\have{br}{\enot\meta{A}}\by{R}{b}
		\close
	\have{d}{\meta{B}}\nelim{c-ar,c-br}
\end{proof}
...
Some have suggested that a different logic ought to be adopted, perhaps a \define{relevance logic} that aims `to make relevance … a necessary condition for the validity of a valid entailment…’.\footnote{Robert K. Meyer (1971) ‘Entailment’, \emph{Journal of Philosophy} \textbf{68}, pp. 808–18, at p. 809.} But such alternative logics have counterintuitive features of their own, including the need to introduce further truth values beyond truth and falsity! Perhaps better to again diminish the bearing of logic as a norm for reasoning: the existence of certain valid implications and acceptance natural deduction rules is no grounds for us to reason or infer in accordance with those rules. (I return to the topic of relevance logic in §\ref{s:modifying}.)


\section{De Morgan Laws}
Our final additional rules are called the \define{De Morgan Laws}. (These are named after the nineteenth century logician Augustus De Morgan.) The first two De Morgan laws show the provable equivalence of a negated conjunction and a disjunction of negations.
...
\paragraph{Con} Adopting DS as a basic rule destroys the nice feature of our standard rules that only one connective is used in any rule. DS needs both disjunction and negation. This has several consequences.
\begin{enumerate}
...
% \problempart Construct two proofs showing that $(A\eif B)\eor (A \eif C)  \proves A \eif (B \eor C)$, the first using our standard natural deduction system, the second using the system which has DS in place of disjunction elimination. Comment on any points of interest.
