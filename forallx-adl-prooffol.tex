...
\section{Why Logic Matters}

We've had to learn many matters of detail in this book, which can sometimes mean we lose sight of our goal. Our original motivation for studying logic was something about determining which arguments are good for use in our inference, and why.

We now have a (partial) answer to that question. 
\factoidbox{An argument that can be represented in \TFL\ or \FOL\ is \define{inferentially regulative} if a symbolisation of it is valid, because such arguments are transparently truth‐preserving.}

By `inferentially regulative' I mean that the argument places a constraint on inference: an ideally rational agent should either accept the conclusion of an inferentially regulative argument, or not accept all of the premises. We've already had some extensive discussion of why logic isn't the totality of a theory of good reasoning (§\ref{ss:reasoning}). Of course sometimes we are not ideally rational: we accept contradictory premises, and lack the time or resources to sort out which we should abandon. In those cases we still we should recognise some sense in which if our beliefs are to be accurate to the world, they must be structured in accordance with notions of entailment.

These arguments are ‘transparent' because everyone who grasps the structure can see that the symbolised argument is valid – no subject‐specific knowledge is required.

This isn't an `iff'. . Some regulative inferences are invalid (e.g., inference to the best explanation). And some good arguments can't be symbolised in \FOL. 


 is more demanding. This is the \define{completeness} of our proof system, that whenever there is a valid argument, there is a proof. It is somewhere around this point, of demonstrating the completeness half of the agreement between  provability and semantic entailment, when introductory logic becomes intermediate logic. More than one of the authors of this book have written distinct sequels in which the completeness of \TFL\ is established, along with other results of a `metalogical' flavour. The interested student is directed to these texts: \begin{itemize}
...
\keyideas{
	\item With the basics under your belt, there are many possible next steps in logic. 
