...
Holding fixed the claim that everything is F, you can conclude that any particular thing is F. You name it; it's F. So the following should be fine:
\begin{proof}
	\hypo{a}{\forall xRxxd}
	\have{c}{Raad} \Ae{a}
\end{proof}
...
Given the assumption that some specific named thing is an F, you can conclude that something is an F: `Sylvester reads, so someone reads' seems like a conclusive argument. So we ought to allow:
\begin{proof}
	\hypo{a}{Raad}
...
We take our cue from the $\forall$E rule. This rule says: take a formula \meta{A}, then we can remove the quantifier and substitute an arbitrary name for some variable. The $\exists$I rule is in some sense a mirror image of this rule: it allows us to move from a sentence with an arbitrary name to a quantified formula. So here is how we formulate our rule of \define{existential introduction}:
\factoidbox{
	\begin{proof}
	...
	\end{proof}
} So really we should think that the proof on the previous page should be thought of as concluding $\exists x Rxxd$ from `$Rxxd$'$\subs{a}{x}$.
...
An argument that makes use of both patterns of arbitrary reasoning is this example due to Breckenridge and Magidor: `from the premise that there is someone who loves everyone to the conclusion that everyone is such that someone loves them'. Here is a proof in our system, letting `$Lxy$' symbolise `\gap{x} loves \gap{y}', letting `$h$' symbolise the arbitrarily chosen name `Hiccup' and letting `$a$' symbolise the arbitrarily chosen name `Astrid': \begin{proof}
...
You may also offer arguments from intepretations to the effect that our other quantifier proof rules correspond to valid arguments in \FOL:\begin{itemize}
	\item $\meta{A}\subs{\meta{c}}{\meta{x}} \entails \exists \meta{x}\meta{A}$;
	\item $\meta{C}_{1},…,\meta{C}_{n},\meta{A}\subs{\meta{c}}{\meta{x}}\entails \exists\meta{x}\meta{A}$, so long as $\meta{c}$ appears nowhere in the argument except in $\meta{A}\subs{\meta{c}}{\meta{x}}$.
\end{itemize} 
...
\chapter{Derived rules in \textnormal{\FOL}: conversion of quantifiers}\label{s:CQ}
...
In §\ref{s:FOLBuildingBlocks}, we noted that $\enot\exists x\meta{A}$ is logically equivalent to $\forall x \enot\meta{A}$. We shall add some rules to our proof system that govern this. In particular, we add two rules, one for each direction of the equivalence:
 ...
 Here is a schematic proof corresponding to our first CQ rule:
\begin{proof}
...
Here is a schematic basic proof emulating the second CQ rule, in which some of the same issues arise:
\begin{proof}
...
Equally, we add rules corresponding to the equivalence of $\exists\meta{x}\enot\meta{A}$ and $\enot\forall\meta{x}\meta{A}$:
\factoidbox{\begin{minipage}{0.35\textwidth}
	\begin{proof}
...
\end{minipage}\qquad\begin{minipage}{0.35\textwidth}
	...
	\end{proof}
\end{minipage}
	}Similar schematic proofs can be offered for the other two CQ rules that show they too can be emulated using just our basic rules.
...
\end{proof} This schematic proof actually justifies this derived rule, to save time: \factoidbox{
	\begin{proof}
		\have[m]{e}{\meta{a}=\meta{b}}
	\have[n]{a}{\meta{A}\subs{\meta{b}}{\meta{x}}}
...
\vbox{
Here is a final proof, that identity is \emph{reflexive}:
\begin{proof}
	\have{a}{a=a}\idi{}
...
\keyideas{
	...
	\item We can prove all the properties we traditionally ascribe to identity – reflexivity, symmetry, and transitivity.
}
