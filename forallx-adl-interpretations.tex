...
\section{Actual and Possible Ingredients for Interpretations}

So far, the domains we have used are drawn from the actual world, and the English predicates are used to assign their actual extensions to \FOL\ predicates. 

But a perfectly good interpretation might assign merely possible entities to the domain, and merely possible extensions to the predicates of \FOL, or a mixture of the two.  Interpretations and possible situations thus can seem to have a lot in common. But there are some subtle differences. If a given possibility shares a domain with the actual world, but the things in it have different properties, then we can make use of purely actual interpretations. For example, the possibility in which Bill Shorten became prime minister after the 2018 Australian federal election can be represented with the actual domain. If we intended to use a predicate `$M$' to represent the property `being prime minister in 2019', we can't use this property to pick out the intended extension, since of course in the actual world, Scott Morrison is in the extension determined by that property. But we can use our other approach of giving an extension directly, and simply assign Bill Shorten as the extension of that predicate. We can assign any extension, whether it corresponds to a natural property or not.

Where non-actual intepretations are useful is in handling possible situations which contain things which didn't actually exist. For example, consider a possibility in which I had another child, called `Eloise'. This might give enable the following interpretation:
\begin{ekey}
	\item[\text{domain}] Antony, Sylvester, Jonquil, Eloise
	\item[C] \gap{1} is a child, i.e., Sylvester, Jonquil, Eloise
	\item[P] \gap{1} is the parent of \gap{2}, i.e., $\ntuple{\text{Antony},\text{Sylvester}},\ntuple{\text{Antony},\text{Jonquil}},\ntuple{\text{Antony},\text{Eloise}}$
\end{ekey}
This entity Eloise is not among the actual inhabitants of the world. (A challenging question, which we will not address, is what enables us to talk about and apparently make use of such a merely possible thing in our interpretations.) It seems preferable, when considering the possibility that I had another child, that we make use of such a `new' entity, rather than reinterpreting `child of Antony' in such a way as to include in its extension some random other actual person! And it will turn out to be essential in understanding counting sentences. Suppose that the actual world contains some finite number of objects, $n$. (I don't have any idea what $n$ is, but I do suspect that it is not infinite.) The maximum domain size for an interpretation drawn from the actual world is thus $n$. So this pure \FOL\ counting sentence that says `there are more than $n$ things' will turn out to be false on \emph{every} interpretation (§\ref{s:atleast}):
$$\exists x_{1}…\exists x_{n+1} (x_{1} ≠ x_{2} \eand … \eand x_{n}≠x_{n+1}).$$ But this sentence is perfectly possible; it doesn't seem like it should be a truth of logic alone that the actual number of things is the upper limit. If we permit interpretations to have domains that range over actual and possible individuals, we won't be subject to any such restriction. Let's therefore make the assumption that interpretations are not restricted to actuality: \factoidbox{Any collection of objects, actual or merely possible, can serve as the domain of an interpretation. Any collection of objects drawn from that domain can serve as the extension of a one-place predicate, any collection of pairs of such objects can serve as the extension of a two-place predicate, etc.}
