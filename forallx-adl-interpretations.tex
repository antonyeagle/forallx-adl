...
It is often convenient to present the extensions associated with an interpretation \emph{diagrammatically}.
...
It is clear that this isn't the only Euler diagram associated with this interpretation. You have a lot of choice about how to draw and shade the various regions. As long as what you produce follows the rules, it doesn't matter. All that matters is that the Euler diagram you produce captures the salient facts about the interpretation you are aiming to represent. 
...
Another example. Consider this interpretation, discussed earlier on page \pageref{poison}: 
	...
	\end{ekey}
The wisdom of Bret Michaels then tells us that the Euler diagram should look like the representation in Figure~\ref{fig:poison}. \begin{figure}
	\begin{tikzpicture}
...
There are many different Euler diagrams (different colours, sizes, and shapes of regions) that can represent a given interpretation. The converse is also true: there are many interpretations that can be represented by a single Euler diagram. Importantly, this can mean that interpretations that disagree over the truth value they assign to a given sentence can be represented by \emph{the same diagram}. 

Consider the simple Euler diagram in Figure~\ref{fig:simpeuler}. This diagram can only represent interpretations in which the extensions assigned to $H$ and $D$ are non-empty, and the intersection of those extensions is also non-empty. But the diagram doesn't tell us how many things are in these non-empty extensions. (The \emph{sizes} of the regions in the diagram have no significance, so we can't deduce that information from the fact that the $H$ region is larger than the $D$ region.) Accordingly, the diagram doesn't correspond to a single interpretation – it could be used to represent both of the following interpretations:
\begin{multicols}{2}
	\begin{enumerate}
	\item \begin{ekey}
	\item[\domain] $0, 1, 2, 3$
	\item[H] \gap{1} is prime, i.e., $\{2,3\}$
	\item[D] \gap{1} is odd, i.e., $\{1,3\}$.
\end{ekey} 
\item \begin{ekey}
	\item[\domain] $0, 1, 2, 3, 4, 5$
	\item[H] \gap{1} is prime, i.e., $\{2,3,5\}$
	\item[D] \gap{1} is odd, i.e., $\{1,3,5\}$.
\end{ekey}
\end{enumerate} 
\end{multicols}
These two interpretations differ in the truth value they assign to some sentences, such as $$\exists x \exists y \left(\left(\left(Hx \eand Dx\right) \eand \left(Hy \eand Dy\right)\right) \eand x \neq y\right).$$ Interpretation 1 makes this sentence false: there is only one thing in the intersection of $H$ and $D$ on that interpretation, namely, $3$. But interpretation 2 makes it true, since $3$ and $5$ are both in the intersection of $H$ and $D$. So if we were to ask, `is that sentence true in that diagram?', there is no determinate answer. The best we could say is that some of the interpretations represented by the diagram make it true, and some of them make it false. So we don't really have any coherent notion of whether a setence is `true according to a diagram' – we have only `true according to an interpretation represented by a diagram'.
...
\begin{figure}[b]
		...
		\node (e) at (2,1.4) {$H$};
		\node (b) at (4,2.5) {$D$};
		\filldraw[pattern color=Salmon, pattern=horizontal lines, draw =black, fill opacity=0.4] (2.3,2) circle (1.4cm);
		\filldraw[pattern=vertical lines, pattern color=Violet, draw =black, fill opacity=0.4] (3.1,2.5) ellipse (1.1cm and 0.75cm);
             \end{tikzpicture}
          \caption{A simple but ambiguous Euler diagram.\label{fig:simpeuler}}    \end{figure}

If you would like a real challenge, you might try to figure out (one of) the interpretation(s) corresponding to the Euler diagram in Figure~\ref{fig:xkcd}. 
\begin{figure}
...
Unlike Euler diagrams, each directed graph represents only one interpretation. If the graph has been correctly constructed, any entity in the domain should have a corresponding node, and the interpretation should assign an extension to only one binary relation, the whole of which is depicted in the graph. It is easy to see there is just one interpretation of a single binary relation for a given domain and extension assigned to that relation.

If we wanted, we could extend our graphical conventions, making our diagrams more complex in order to depict more complex interpretations. For example, we could introduce another kind of arrow (maybe with a dashed shaft) to represent a further two-place predicate.\footnote{This kind of `directed multigraph' is sometimes called a \define{quiver}.} We could add names as labels attached to particular objects. To symbolise the extension of a one-place predicate, we might simply adopt the approach from §\ref{euler}, and mark a shaded region around some particular objects and stipulate that the thus encircled objects (and only them) are to fall in the extension of some predicate `$H$', say.\footnote{We needn't stop there. For example, we could introduce \define{directed hypergraphs}, where the edges can join more than two nodes.} 
...
Let us start, as before, by giving rules for evaluating atoms of \FOL, given an interpretation and a variable assignment. \factoidbox{Where $\meta{R}$ is any $n$-place predicate ($n ≥ 0$), and $\meta{t}_{1},…,\meta{t}_{n}$ are any terms – variables or names, then: \begin{itemize}
...
\problempart
% \label{pr.TorF3}
Following the diagram conventions introduced at the end of §\ref{s:Interpretations}, consider the interpretation represented by the following graph:	
\begin{center}
\begin{tikzpicture}
\node (atom1) at (0,2) {1};
\node (atom2) at (2,2) {2};
