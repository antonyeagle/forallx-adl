...
We can stipulate directly which things in our domain our predicates are to be true of. So it is worth noting that our stipulations can be as arbitrary as we like. For example, we could stipulate that `$H\meta{x}$' should be true of, and only of, the following objects:
	\begin{center}
		David Cameron\\
		the number $\pi$\\
...
Given what I said above, this symbolisation key should be read as saying:
	\begin{earg}
		\item[\textbullet] `$L\meta{xy}$' and `\gap{x} loves \gap{y}' are to be true of exactly the same things in the domain
	\end{earg}
So, in particular:
	\begin{earg}
...
To do this, we can specify that two-place predicates are true of \define{ordered pairs} of objects, which differ from two-membered collections in that the order of a pair is important. Thus we might stipulate that `$B\meta{xy}$' is to be true of, and only of, the following pairs of objects:
	\begin{center}
		\ntuple{Lenin, Marx}\\
...
I defined a \emph{valuation} in \TFL\ as any assignment of truth and falsity to atomic sentences. In \FOL, I am going to define an \define{interpretation} as consisting of three things:
	\begin{itemize}	
		...
		\item for each non-logical $n$-place predicate $\meta{Ax}_{1}…\meta{x}_{n}$ (i.e., any predicate other than `$=$') that we care to consider, a specification of its extension: what things (or pairs of things, or triples of thing, etc.) the predicate is to be true of – where all those things must be in the domain.
	\end{itemize}
...
\noindent\begin{tabular}{p{0.36\textwidth} p{0.58\textwidth}}
\begin{ekey}
	\item[\text{domain}] Days of the week
	\item[B\meta{xy}] \gap{x} is the day before \gap{y}
\end{ekey} & \begin{ekey}
...
\item The interior of a region associated with predicate $\meta{Px}$ represents the members of the domain that are in the extension of $\meta{Px}$; the exterior – the region within the domain but outside the given region – represents those members of the domain that are not in the extension of $\meta{Px}$;
...
These rules are quite abstract, but they are easy to apply in practice. Suppose we are given the following interpretation: \begin{ekey}
	...
	\item[E\meta{x}] \gap{x} lays eggs;
	\item[M\meta{x}] \gap{x} is a mammal;
	\item[B\meta{x}] \gap{x} is a bird;
\end{ekey}
...
Suppose we want to consider just a single two-place predicate, `$R\meta{xy}$'. Then we can represent it, in some cases, by depicting the members of the domain by writing them down, and drawing a single-headed arrow between items just in case the relation holds between them (in the right order). Such a diagram is known as a \define{directed graph}, which is given by a collection of \emph{nodes} (the objects in the domain we are representing) and a collection of arrows between them (which represent how the relation holds on that domain.) We stipulate that `$R\meta{xy}$' is to hold of \meta{x} and \meta{y} just in case there is an arrow running from \meta{x} to \meta{y} in our diagram. As an example, consider the following interpretation, written in the more standard manner:
\begin{ekey}
	\item[\text{domain}] $1, 2, 3, 4$
	\item[R\meta{xy}] \ntuple{1, 2}, \ntuple{2, 3}, \ntuple{3, 4}, \ntuple{4, 1}, \ntuple{1, 3}.
\end{ekey}
That is, an interpretation whose domain is the first four positive whole numbers, and which interprets `$R\meta{xy}$' as being true of and only of the specified pairs of numbers. This might be represented by the following simple graph:
\begin{center}
\begin{tikzpicture}
\node (atom1) at (0,2) {1};
\node (atom2) at (2,2) {2};
...
\end{tikzpicture}
\end{center} Notice that 2 is in the domain, and hence included as a node in the graph, but it has no arrows attached to it, because it is not included in the relation assigned to $R\meta{xy}$. 
...
Consider how we we might graphically represent the following interpretation, which will make use of all these graphical innovations: \begin{ekey}
	...
	\item[G\meta{x}] $\gap{x} \geqslant 4$. 
	\item[D\meta{xy}] \gap{x} is distinct from and exactly divisible by \gap{y};
	\item[T\meta{xy}] $\gap{x} + 3 = \gap{y}$;
	\item[f] $4$;
\end{ekey} 
This graph represents that interpretation, where the label `$f$' represents the name attached to 4, the blue dotted arrow represents the relation assigned to $T\meta{xy}$, the black solid arrow represents the relation assigned to $D\meta{xy}$, and the grey ellipse represents the collection of things in the domain which have the property assigned to $G\meta{x}$:
\begin{center}
\begin{tikzpicture}
\node (atom2) at (1,2) {2};
\node (atom3) at (3,2) {3};
...
Using some of the methods introduced in this section, give a graphical representation of the following interpretations. You will need to rely on your general knowledge to prepare these diagrams. \begin{earg}
	\item \begin{ekey}
		...
		\item[M\meta{x}] \gap{x} is a musician;
		\item[W\meta{x}] \gap{x} is a woman;
		\item[E\meta{x}] \gap{x} is an economist.
	\end{ekey}
	\item \begin{ekey}
\item[\text{domain}] cards in a standard deck
\item[S\meta{x}] \gap{x} is a seven;
\item[F\meta{x}] \gap{x} is a face card;
\item[R\meta{x}] \gap{x} is red;
\item[j] the Jack of Hearts.
	\end{ekey}
	\item \begin{ekey}
		\item[\text{domain}] Australian states
		\item[L\meta{xy}] \gap{x} is larger in population than \gap{y};
		\item[V\meta{x}] \gap{x}'s name ends in a vowel;
