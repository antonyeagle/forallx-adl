...
\section{Zero-place predicates}
 ...
 \begin{frame}\frametitle{Zero-Place Predicates}
    \begin{itemize}[<+->]
    ...
    \end{enumerate}
    \item In 1, `it' refers to Coober Pedy. Not so in 2: `It' in that sentence is a \alert{dummy pronoun} (though `there' does refer)!

    \item We took this example to suggest that the \alert{bare} report `it is raining' involved no genuine singular term, and modelled by a \alert{zero-place predicate} like $R$. 
    	...
    	\item We treat them as having \alert{truth values as their extension} – just like the \alert{valuations} of \TFL. (In fact we can consider the valuations of \TFL\ as assignments of truth values as the extensions of atomic sentences of that language.)
    \end{itemize}
    \end{frame}
...
We can start to see how interpretations can help us evaluate arguments quite vividly in this graphical environment. Recall this example from §\ref{s:FOLBuildingBlocks}: `Willard (`$w$') is a logician (`$L$'); Every logician wears a funny hat (`$F$') \ttherefore\ Willard wears a funny hat'. We can see this is valid: Willard falls in the region corresponding to `$F$' because he falls in the region corresponding to `$L$', which is wholly within the `$F$'-region. \begin{column}
             \begin{tikzpicture}
...
A third example, corresponding to this interpretation: \begin{ekey}
...
\problempart
Show that each of the following is either a logical truth or a contradiction:
\begin{earg}
