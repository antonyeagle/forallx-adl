...
We can start to see how interpretations can help us evaluate arguments quite vividly in this graphical environment. Recall this example from §\ref{s:FOLBuildingBlocks}: `Willard (`$w$') is a logician (`$L$'); Every logician wears a funny hat (`$F$') \ttherefore\ Willard wears a funny hat'.  This can be represented using an Euler diagram making use of a labelled dot to represent the individual Willard, as in Figure~\ref{fig:willard}. We can see from the diagram that this argument is valid: Willard falls in the region corresponding to `$F$' because he falls in the region corresponding to `$L$', which is wholly within the `$F$'-region. \begin{figure}
             \begin{tikzpicture}
              	\draw[thick] (0,0) -- (0,4) -- (5,4) -- (5,0) -- cycle;
...
\factoidbox{\begin{itemize}
	\item A binary relation $\symcal{R}$  on $D$ is \define{reflexive} iff for any $x$ in $D$, $\ntuple{x,x}$ is in $\symcal{R}$. 
	\item $\symcal{R}$ is \define{irreflexive} on $D$ iff for no $x$ in $D$ is $\ntuple{x, x}$ in $\symcal{R}$.
	\item If $\symcal{R}$ is neither reflexive nor irreflexive, it is nonreflexive.
\end{itemize}}
An example of a reflexive relation is the extension of `\gap{1} is the same height as \gap{2}' on the domain of people. Everyone is the same height as themselves. An example of an irreflexive relation is the extension of `\gap{1} is taller than \gap{2}' on the domain of people, since no one is taller than they are. An example of a nonreflexive relation might be the extension of `\gap{1} trusts the judgment of \gap{2}' on the domain of people: some people trust their own judgement, while others second-guess themselves. Pictorially, a reflexive relation corresponds to a graph in which every node in the graph has an arrow pointing to itself, such as in Figure~\ref{fig:reflex}. 
\begin{figure}[t]
\begin{tikzpicture}
\node (atom1) at (0,2) {1};
\node (atom3) at (2,0) {3};
...
Reflexivity is a property of an extension, not a predicate (except the identity predicate). But there is a sentence of \FOL\ which expresses reflexivity. Suppose the two-place predicate `$R$' is assigned a relation $\symcal{R}$, a set of pairs, as its extension on $D$: then `$\forall x Rxx$' (`everything bears $R$ to itself') will be true under this interpretation iff $\symcal{R}$ is reflexive. 

\factoidbox{\begin{itemize}
	\item A binary relation $\symcal{R}$ is \define{transitive} iff for any $x$, $y$, and $z$, whenever $\ntuple{x, y}$ and $\ntuple{y, z}$ are in $\symcal{R}$, then $\ntuple{x, z}$ is also in $\symcal{R}$.
	\item $\symcal{R}$ is \define{intransitive} iff for any $x$, $y$, and $z$, whenever $\ntuple{x, y}$ and $\ntuple{y, z}$ are in $\symcal{R}$, then $\ntuple{x, z}$ is \emph{not} in $\symcal{R}$.
	\item $\symcal{R}$ is nontransititive iff it is neither transitive nor intransitive.
\end{itemize} If the predicate `$R$' is assigned $\symcal{R}$ as its extension in some interpretation, then $\symcal{R}$ is transitive iff `$\forall x \forall y \forall z ((Rxy \eand Ryz) \eif Rxz)$' holds in this interpretation.}
...
These properties of binary relations are not completely independent of each other. For example, a reflexive relation cannot be asymmetric: a relation $\symcal{R}$ is asymmetric only if it is never the case that for any $x$ and $y$, $\ntuple{x,y}$ is in $\symcal{R}$; but if the relation is reflexive, then every pair consisting of something and itself is in $\symcal{R}$, and nothing prohibits us from picking the same value for $x$ and $y$. A reflexive relation can at best be antisymmetric. 
...
A relation on a domain $D$ is \define{total} iff for any $x$ and $y$ in $D$, either $\ntuple{x,y}$ or $\ntuple{y,x}$ (or both) is in the extension.\footnote{If the predicate `$R$' is assigned $\symcal{R}$ as its extension in some interpretation, then $\symcal{R}$ is total iff `$\forall x \forall y (Rxy \eor Ryx)$' holds in this interpretation.} Any two things are related, in some way, by a total relation.  We can use the notion of totality to define a kind of relation that is of particular mathematical importance: \factoidbox{A relation $\symcal{R}$ on a domain $D$ is an \define{ordering} iff $\symcal{R}$ is reflexive, transitive, and antisymmetric. Special cases: \begin{itemize}
	\item $\symcal{R}$ is a \define{strict order}  iff $\symcal{R}$ is an asymmetric order; i.e., iff $\symcal{R}$ is reflexive, transitive, and asymmetric.
	\item $\symcal{R}$ is a \define{total order}  iff $\symcal{R}$ is total and is an order;
	\item $\symcal{R}$ is a \define{strict total order}  iff $\symcal{R}$ is total and is a strict order.
...
\keyideas{
	\item An interpretation of \FOL\ is a temporary assignment of extensions to some of the nonlogical vocabulary of \FOL\, i.e., the names and predicates of the language.
...
\problempart \begin{earg} \item Show that any total relation on a domain is reflexive on that domain.
...
\item What is wrong with the following argument that reflexivity is a consequence of symmetry and transitivity? \begin{quote}
	If $\ntuple{x,y}$ is in $\symcal{R}$, then $\ntuple{y,x}$ is in $\symcal{R}$ since we assume $\symcal{R}$ is symmetric. If both $\ntuple{x,y}$ is in $\symcal{R}$ and $\ntuple{y,x}$ is in $\symcal{R}$, then since $\symcal{R}$ is transitive, $\ntuple{x,x}$ is in $\symcal{R}$ – so $\symcal{R}$ is reflexive. 
\end{quote}
\item A relation $\symcal{R}$ is \define{serial} on a domain $D$ iff for each $x$ in $D$, there exists some $y$ such that $\ntuple{x,y}$ is in $\symcal{R}$.  Show that a serial, symmetric and transitive relation is reflexive. 
\end{earg}
...
As before, there is a close connection between validity and entailment: \factoidbox{
 $\meta{A}_1, \meta{A}_2, … \meta{A}_n \ttherefore \meta{C}$ is \define{valid} in \FOL\ iff there is no interpretation in which all of the premises are true and the conclusion is false; i.e., $\meta{A}_1,\meta{A}_2,… \meta{A}_n \entails\meta{C}$. It is \define{invalid} in \FOL\ otherwise.} Our entailments are conclusive in virtue of form, because the consideration of many alternative interpretations of the nonlogical expressions involved means that no argument can be conclusive because of some special `connection in meaning' between such expressions in the language. Entailment in \FOL\ reflects only the logical structure of the sentences involved, where this now includes the new logical resources of \FOL\ over \TFL.
...
\keyideas{
	...
	\item The notion of a logical truth in \FOL\ is slightly different than the notion of a logical truth, since we allow interpretations that vary not just in what they assign as extensions to the nonlogical vocabulary, but also variation in the domain over which quantifiers range. Nothing like the latter feature occurs in \TFL\ valuations, but it is crucial for ensuring that contingent truths without names or nonlogical predicates aren't misclassified as logical truths.
}
...
\end{quote} This is an example of \define{\emph{reductio}} reasoning\phantomsection\label{reductio}: we assume something, and derive some absurdity or contradiction; we then conclude that our assumption is to be rejected. It is often easier to show that all Fs have a property by deriving an absurdity from the assumption that some F lacks the property, than it is to show it `directly' by considering each F in turn. This is a powerful technique, used widely in mathematics and elsewhere.
...
% {\HUGE These still need to be done \causeanerror}
